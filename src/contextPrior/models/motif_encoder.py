...
class SwiGLUFeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = torch.nn.functional.silu(self.gate_proj(x)) * self.value_proj(x)
        x = self.out_proj(x)
...
class AxialSelfAttention1D(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            mask_value = -1e4 if x.dtype in (torch.float16, torch.bfloat16) else -1e9
...
class MotifTAxisAttention(nn.Module):
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        super().__init__()
        self.attn = AxialSelfAttention1D(
            dim=dim,
            num_heads=num_heads,
...
class MotifFAxisAttention(nn.Module):
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class MotifAxialLayer(nn.Module):
    def __init__(self, cfg: MotifInferenceConfig):
        ...
        self.t_attn = MotifTAxisAttention(
            dim=hidden_dim,
        ...
        self.f_attn = MotifFAxisAttention(
            dim=hidden_dim,
            num_heads=cfg.motif_encoder_num_attention_heads,
            attention_dropout=cfg.motif_encoder_attention_dropout,
            bias=cfg.motif_encoder_attn_bias,
...
class MotifAxialEncoder(nn.Module):
    def __init__(self, cfg: MotifInferenceConfig):
        super().__init__()
        self.cfg = cfg
        self.layer = nn.ModuleList([MotifAxialLayer(cfg) for _ in range(cfg.motif_encoder_num_hidden_layers)])
